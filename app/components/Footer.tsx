    ...
    <div className="z-[100] h-10 bg-[#030712] border-t border-white/5 px-4 md:px-6 flex items-center justify-between text-[11px] font-medium text-white/30 overflow-hidden select-none shrink-0 border-b-none">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="hidden sm:flex items-center gap-2 opacity-30 group">
          <Cpu size={12} className="group-hover:text-[#22d3ee] transition-colors" />
      ...
      <div className="hidden lg:flex items-center gap-8">
        {[
          ...
          <div key={idx} className="flex items-center gap-1.5 hover:text-[#22d3ee] transition-all duration-300 cursor-pointer group">
            <span className="text-white/10 group-hover:text-[#22d3ee]/50 transition-colors">{item.icon}</span>
      ...
      <div className="flex items-center gap-4">
        <div className="hidden md:flex items-center gap-4 pr-4 border-r border-white/5">
          {[Github, Twitter, Linkedin, Mail].map((Icon, idx) => (
            <Icon key={idx} size={13} className="hover:text-[#22d3ee] transition-all duration-300 cursor-pointer hover:-translate-y-0.5" />
          ))}
        </div>
